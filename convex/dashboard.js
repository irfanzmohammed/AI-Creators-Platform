...
for (const postId of postIds) {
  ...
  const filtered = comments.filter(
    (comment) => comment.createdAt > thirtyDaysAgo
  );

  recentComments += filtered.length;
}

// Get recent followers count (last 30 days)
const recentFollowers = followersCount.filter(
  (follow) => follow.createdAt > thirtyDaysAgo
);
    ...
    const commentsGrowth =  totalComments > 0 ? (recentComments / totalComments) * 100 : 0;
    const followersGrowth = followersCount.length > 0
    ? (recentFollowers.length / followersCount.length) * 100
    : 0;
